#!/bin/bash
# lansing.love deploy — build on Mac, rsync to Pi
# Usage: ./deploy.sh
# Run prisma migrate deploy + prisma generate on the Pi FIRST if the schema changed.
...
echo "==> Done. Verify: curl -s http://localhost:3001 | grep -o '<title>[^<]*'"
